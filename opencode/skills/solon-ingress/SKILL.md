---
name: solon-ingress
description: High-quality memory ingestion skill for Solon decisions with quality gate and co-loaded graphiti utilities
...
Loaded by a sub-agent (Sonnet, `category='unspecified-high'`). Receives pre-classified decisions from solon-spec Phase 5 batch ingestion.

Co-loads: `graphiti-enhancer`, `graphiti-ledger-insert`, `graphiti-normalizer`.
All three are called **inline** — do NOT dispatch sub-sub-agents (depth 1 nesting blocked).
...
- **source_description template**: `platform:opencode agent:solon session:{id} repo:{repo} model:{model}`
...
```
platform:opencode agent:solon session:{id} repo:{repo} model:{model} GUID:{uuid}
```
