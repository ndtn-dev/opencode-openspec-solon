...
description: Drains pending JSON fallbacks, verifies episodes in FalkorDB, and reports ledger status. Always delegated to a sub-agent to prevent context bloat.
...
Main-agent role: load this skill to follow delegation instructions only. Do not run drain/verify/report directly in the main-agent context.

## Delegation Pattern

Always dispatch to a sub-agent with `task()` and `load_skills`:

```python
task(
  category='quick',
  load_skills=['graphiti-ledger-status'],
  run_in_background=true,
  prompt='CHECK LEDGER STATUS: [drain|verify|report|all] for session {session_id}'
)
```

Sub-agent executes one operation (`drain`, `verify`, `report`) or full sequence (`all`).
...
3. If insert succeeds, delete pending file:
   - `Bash("rm .graphiti/ingress/pending/{uuid}.json")`
4. Output:
...
```python
# Get FalkorDB episode created_at — use get_episodes filtered by group_id,
# or extract created_at from the search result metadata if available.
...
- FalkorDB ingestion is asynchronous; local models (for example `qwen3:17b`) can process slowly.
...
- Keep main-agent context thin: all heavy operations run in delegated sub-agent.
