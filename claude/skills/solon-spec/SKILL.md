---
name: solon-spec
description: Core spec-writing workflow with 7-phase process, soft ratchet, and ingress dispatch. Invoke when Solon routes to Spec, Plan-to-spec, Exploratory, Explicit, or Open-ended intent.
---
...
1. Use `graphiti-ledger-insert` to insert decision row with `phase=P2`, `tier=Big`, `decision_status=active`.
2. Call `add_memory` for the confirmed Big decision.
3. Keep GUID linkage in source metadata.
...
- Invoke `graphiti-ledger-status` and verify current-session rows.
- Capture verified vs pending counts.

2) Batch-record confirmed Small/Medium decisions:
- Use `graphiti-ledger-insert` to insert confirmed Small/Medium assumptions for first-time ingestion tracking.
...
"INGRESS CHECKPOINT BATCH: process confirmed Phase 5 decisions, enforce Decision #19 all-tier ingestion, skip Big graph duplicates already ingested in Phase 2, return enhanced/dropped/ingested summary with errors if any."
The agent should use the solon-ingress, graphiti-enhancer, graphiti-ledger-insert, and graphiti-normalizer skills.
Depth guard:
...
1. Dispatch a background Agent with the graphiti-ledger-status skill to run a full ledger pass (drain, verify, report) for the current session.
